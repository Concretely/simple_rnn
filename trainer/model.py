...
def train_and_evaluate(args, output_dir, use_keras):
    if use_keras:
       estimator = make_keras_estimator(output_dir)
    else:
       estimator = tf.estimator.Estimator(model_fn = simple_rnn, 
                                          model_dir = output_dir)
    train_spec = tf.estimator.TrainSpec(read_dataset(args.train_data_paths + 'train.csv',
                                            tf.estimator.ModeKeys.TRAIN,
                                            512),
                                        max_steps = 1000)
    exporter = tf.estimator.LatestExporter('exporter', serving_input_fn)
    eval_spec = tf.estimator.EvalSpec(read_dataset(args.eval_data_paths+'valid.csv',
